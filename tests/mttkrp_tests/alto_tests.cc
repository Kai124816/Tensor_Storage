...
void mttkrp_on_tensor(std::string filename, std::vector<int> dims, int nnz, int mode, std::string diff_file = "differences.txt")
{
    ...
    int decomp_rank = alto.get_factor_rank();
    ...
    if constexpr (std::is_floating_point<T>::value){
        double diff = compare_arrays_float<T>(output_matrix, test_matrix, dims[mode-1] * decomp_rank);
        if(diff > 0.00001) test_passed = false;
        else test_passed = true;
        std::cout << "Average difference of: " << diff << "\n";
    }
    else test_passed = compare_arrays<T>(output_matrix, test_matrix, dims[mode-1] * decomp_rank);
    ...
    if(test_passed) std::cout << "Tests Passed!\n";
            ...
            if constexpr (std::is_floating_point<T>::value) {
                print_differences_to_file_float(output_matrix, test_matrix, dims[mode - 1], decomp_rank, diff_file, "ALTO Matrix", "Test Matrix");
            }
            else {
                print_differences_to_file(output_matrix, test_matrix, dims[mode - 1], decomp_rank, diff_file, "ALTO Matrix", "Test Matrix");
            }           
    }
